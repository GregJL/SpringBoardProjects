...
async function setupAndStart() {
	...
	const resCategories = await axios.get("https://rithm-jeopardy.herokuapp.com/api/categories?count=100");
	let catIds = getCategoryIds(resCategories);

	for (id of catIds) {
		// for each id, get clue data from jservoce.io
		const resTitles = await axios.get("https://rithm-jeopardy.herokuapp.com/api/category?id="+id);
